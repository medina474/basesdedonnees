...
create unique index film_genre_pkey
  on films_genre
  using btree (film_id, genre_id);

alter table films_genre
  add primary key
  using index film_genre_pkey;

-- films_genre -> films
create index on films_genre(film_id);
...
-- films_genre -> genre
create index on films_genre(genre_id);
...
create or replace view tests.films_genre_sans_film as
  select fg.film_id,
    fg.genre_id
  from (films_genre fg
...
RETURNS TABLE(film_id int, titre text, titre_original text,
...
BEGIN
  return query select f.film_id, f.titre, f.titre_original,
      f.annee, f.sortie, f.duree, v.votants, v.moyenne,
      f2.serie
    ...
    left join series f2 on f2.serie_id = f.serie_id
    left join votes v on v.film_id = f.film_id
    left join lateral (SELECT r2.resume FROM resumes r2 WHERE r2.film_id  = f.film_id
      order by array_position(array['deu','fra','eng'], r2.langue)
    ...
    where e.personne_id = id::int
    group by f.film_id, f.titre, f.titre_original, f2.serie, v.votants, v.moyenne, e.ordre, r.resume
    order by e.ordre, v.votants desc;
END;
$$
...
copy films_genre (film_id, genre_id)
